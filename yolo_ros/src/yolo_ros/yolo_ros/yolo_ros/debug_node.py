...
class DebugNode(LifecycleNode):
    ...
    def draw_box(
        ...
        if detection.bbox3d.frame_id:  # Check if 3D bounding box data exists
            x = detection.bbox3d.center.position.x
            y = detection.bbox3d.center.position.y
            z = detection.bbox3d.center.position.z
            label = f"({x:.2f},{y:.2f},{z:.2f})"  # Format X, Y, Z coordinates
        else:
            label = f"{class_name}"  # Default to class name if no 3D data
            label += " ({:.3f})".format(score)

        pos = (min_pt[0] + 5, min_pt[1] + 25)
        font = cv2.FONT_HERSHEY_SIMPLEX
        # cv2.putText(cv_image, label, pos, font, 1, color, 2, cv2.LINE_AA) # UNCOMMENT THIS TO HAVE LABELS
    ...
    def draw_mask(
        ...
        if mask_msg.data:
            ...
            cv2.addWeighted(cv_image, 0.7, layer, 0.3, 0, cv_image)
    ...
    def create_bb_marker(self, detection: Detection, color: Tuple[int]) -> Marker:
        ...
        marker.color.r = color[0] / 255.0  # Red channel
        marker.color.g = color[1] / 255.0  # Green channel
        marker.color.b = color[2] / 255.0  # Blue channel
        marker.color.a = 0.4 # THIS IS THE OPACITY OF THE 3D BOX
    ...
    def detections_cb(self, img_msg: Image, detection_msg: DetectionArray) -> None:
        ...
        for detection in detection_msg.detections:
            ...
            # Use the get_class_color method to get a consistent color
            color = self.get_class_color(detection)

            # Draw the 2D bounding box, mask, and keypoints using the color
    ...
    def get_class_color(self, detection: Detection) -> np.ndarray:
        if detection.class_id == 0: color = (0, 0, 139)  # Dark blue
        elif detection.class_id == 1: color = (0, 100, 0)  # Dark green
        elif detection.class_id == 2: color = (255, 100, 0)  # Dark orange
        elif detection.class_id == 3: color = (128, 0, 128)  # Dark purple
        elif detection.class_id == 4: color = (139, 0, 0)  # Dark red
        elif detection.class_id == 5: color = (255, 200, 0)  # Dark yellow (golden yellow)
        return color
